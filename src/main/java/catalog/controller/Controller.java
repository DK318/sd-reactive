...
import catalog.domain.Currency;
import catalog.domain.Product;
import catalog.domain.User;
import io.netty.buffer.ByteBuf;
import io.netty.handler.codec.http.cookie.Cookie;
import io.netty.handler.codec.http.cookie.DefaultCookie;
import io.reactivex.netty.protocol.http.server.HttpServerRequest;
import io.reactivex.netty.protocol.http.server.HttpServerResponse;
import catalog.MongoDriver;
import catalog.view.HTMLRenderer;
        ...
        switch (req.getDecodedPath()) {
            case "/register":
                return parameters.map(m -> {
                    ...
                    catalog.domain.Currency currency = Currency.valueOf(m.get("currency").toUpperCase(Locale.ROOT));
